...
class FFEApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Friend File Encryptor - Version NR - ffe_pr_04212025_nr")
    ...
    def create_widgets(self):
        ...
        self.drive_menu.pack(side=tk.LEFT, padx=60, expand=True, fill=tk.X)
        ...
        encrypt_button.pack(side=tk.LEFT, padx=7, pady=8)
        ...
        decrypt_button.pack(side=tk.LEFT, padx=7, pady=8)

        delete_button = HoverButton(toolbar, text=" Delete ", command=self.delete_file, bg="#bf3e3b", fg="white", relief="flat", font=("Segoe UI", 11))
        delete_button.pack(side=tk.LEFT, padx=7, pady=8)

        update_button = HoverButton(toolbar, text=" Update ", command=self.update_ffe, bg="#203161", fg="white", relief="flat", font=("Segoe UI", 11))
        update_button.pack(side=tk.LEFT, padx=7, pady=8)

        help_button = HoverButton(toolbar, text=" Help ", command=self.show_help, bg="#203161", fg="white", relief="flat", font=("Segoe UI", 11))
        help_button.pack(side=tk.LEFT, padx=7, pady=8)

        about_button = HoverButton(toolbar, text=" About ", command=self.show_about, bg="#203161", fg="white", relief="flat", font=("Segoe UI", 11))
        about_button.pack(side=tk.LEFT, padx=7, pady=8)

        self.status_label = tk.Label(self, text="Select a file to encrypt. Double click folders to navigate.", bg="#0a1124", fg="white", font=("Segoe UI", 11)) #FONT C
...
FFE Pre-Release Build

Version NR
Build: ffe_pr_04212025_nr
Build Date: 04/21/2025
Ident: 4625L830NTK200FE13LYRA23784
...
All Rights Reserved.
    ...
    def show_help(self):
     response = messagebox.askyesno("Need Help?", "Help is available on the official FFE GitHub! Open a new Issue or start a new discussion to get help there! Want to go there now?")
     if response == tk.YES:
        webbrowser.open_new("https://github.com/AVXAdvanced/FFE") # Changed to your actual repo link!
    ...
    def update_ffe(self):
        ...
        try:
           ...
           if version.parse(latest_version) > version.parse(current_version):
               ...
               self.status_label.config(text="You're already up to date. No updates currently availible.")
