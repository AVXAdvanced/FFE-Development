...
class FFEApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("LYRA DEVELOPMENT BETA - VERSION N6")
        self.geometry("995x600")
        self.configure(bg="#1e1c29") #CLR-BG-BG-A
        ...
        self.file_listbox = tk.Listbox(self, bg="#1e1c29", fg="white", selectmode=tk.SINGLE, font=("Arial", 12)) #CLR-BG-F/SEL-B
        self.create_widgets()
        self.load_files()
    ...
    def create_widgets(self):
        ...
        encrypt_button = HoverButton(toolbar, text="Encrypt", command=self.encrypt_file, bg="#469c57", fg="white", relief="flat", font=("Arial", 12))
        encrypt_button.pack(side=tk.LEFT, padx=8, pady=8)

        decrypt_button = HoverButton(toolbar, text="Decrypt", command=self.decrypt_file, bg="#469c57", fg="white", relief="flat", font=("Arial", 12))
        decrypt_button.pack(side=tk.LEFT, padx=8, pady=8)

        delete_button = HoverButton(toolbar, text="Delete", command=self.delete_file, bg="#bf3e3b", fg="white", relief="flat", font=("Arial", 12))
        delete_button.pack(side=tk.LEFT, padx=8, pady=8)

        help_button = HoverButton(toolbar, text="Help", command=self.show_help, bg="#343047", fg="white", relief="flat", font=("Arial", 12))
        help_button.pack(side=tk.LEFT, padx=8, pady=8)

        about_button = HoverButton(toolbar, text="About", command=self.show_about, bg="#343047", fg="white", relief="flat", font=("Arial", 12))
        about_button.pack(side=tk.LEFT, padx=8, pady=8)

        self.status_label = tk.Label(self, text="Select a file to encrypt. Double click folders to navigate.", bg="#1e1c29", fg="white", font=("Arial", 10))
...
VERSION N6 (N6-04032025-LYRA-IDB)
FFE0403LYRA-IDB
